...
<v-container fluid>
    <v-row align="center" justify="center">
        <v-col cols="12" md="8">
            ...
            <form-header id="createForm" css="" action="{{ route('contacts.store') }}" method="post" enctype=""
                token="{{ csrf_token() }}">
                <custom-field cname="name" cvalue="{{ old('name') }}" :rules="[
                    v => !!v || 'Name is required',
                    v => (v && v.length >= 5) || 'The name must be at least 5 characters.']" clabel="Name" required>
                </custom-field>

                <custom-field cname="contact" cvalue="{{ old('contact') }}" mask="#########" :rules="[
                    v => !!v || 'Contact is required',
                    v => (v && v.length > 8) || 'Contact must be 9 digits',]" clabel="Contact" required></custom-field>

                <custom-field cname="email" cvalue="{{ old('email') }}" :rules="[
